...
WRITE_DIR         = REPO_DIR if os.name == "nt" else "/tmp"
...
def query_history(date_from: str, date_to: str, campaigns: list, judgments: list) -> pd.DataFrame:
    with get_db() as conn:
        ...
        sql = "SELECT * FROM history WHERE 날짜 BETWEEN ? AND ?"
        if campaigns:
            sql += " AND 캠페인 IN ({})".format(",".join("?" * len(campaigns)))
            params += campaigns
        if judgments:
            sql += " AND 판단 IN ({})".format(",".join("?" * len(judgments)))
            params += judgments
        sql += " ORDER BY 날짜 DESC, 비용 DESC"
...
def load_csv(file_bytes: bytes, campaign_name: str) -> pd.DataFrame:
    ...
    for enc in ("utf-8-sig", "utf-8", "shift-jis", "cp932"):
        try:
            ...
            continue
    if df is None:
        st.error(f"[{campaign_name}] CSV 파일을 읽을 수 없습니다.")
        return pd.DataFrame()
    ...
    for col in ["검색어", "매치타입", "비용", "구매수", "매출", "ACOS", "ROAS"]:
...
tab1, tab2, tab3 = st.tabs(["📥 데이터 업로드", "🔍 제외 키워드 검토", "📋 히스토리 조회 · 내보내기"])
        ...
        edited = st.data_editor(
            ...
            column_config={
                "판단": st.column_config.SelectboxColumn(
                    "판단", options=JUDGMENT_OPTIONS, required=True, width="large",
                ),
                "상품링크": st.column_config.LinkColumn(
                    "상품링크", display_text="🔗 열기", width="small",
                ),
            },
...
with tab3:
    ...
    if st.session_state.campaigns:
        ...
        with get_db() as _conn:
            already = pd.read_sql(
                "SELECT COUNT(*) as cnt FROM history WHERE 날짜=?", _conn, params=[today_str]
            ).iloc[0]["cnt"]
